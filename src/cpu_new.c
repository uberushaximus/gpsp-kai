...
static u32 *rom_branch_hash[ROM_BRANCH_HASH_SIZE];

static u8 rom_translation_cache[ROM_TRANSLATION_CACHE_SIZE];
static u8 *rom_translation_ptr = rom_translation_cache;

static u8 ram_translation_cache[RAM_TRANSLATION_CACHE_SIZE];
static u8 *ram_translation_ptr = ram_translation_cache;
static u32 iwram_code_min = 0xFFFFFFFF;
static u32 iwram_code_max = 0xFFFFFFFF;
static u32 ewram_code_min = 0xFFFFFFFF;
static u32 ewram_code_max = 0xFFFFFFFF;

static u8 bios_translation_cache[BIOS_TRANSLATION_CACHE_SIZE];
static u8 *bios_translation_ptr = bios_translation_cache;

static s32 translate_block_arm(u32 pc,
 TRANSLATION_REGION_TYPE translation_region, u32 smc_enable);
static s32 translate_block_thumb(u32 pc,
 TRANSLATION_REGION_TYPE translation_region, u32 smc_enable);
...
static u8 *ram_block_ptrs[1024 * 64];
static u32 ram_block_tag_top = 0x0101;

static u8 *bios_block_ptrs[1024 * 8];
static u32 bios_block_tag_top = 0x0101;
...
static u32 translation_recursion_level = 0;
static u32 translation_flush_count = 0;
